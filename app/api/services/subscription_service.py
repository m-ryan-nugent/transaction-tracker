...
async def create_subscription(
    db: aiosqlite.Connection, subscription: SubscriptionCreate
) -> SubscriptionResponse:
...
async def get_all_subscriptions(
    ...
    total_monthly = sum(
        calculate_monthly_cost(s.amount, s.billing_cycle) for s in active_subs
    )
...
async def get_upcoming_renewals(
    db: aiosqlite.Connection, days_ahead: int = 30, limit: int = 10
...
async def update_subscription(
    db: aiosqlite.Connection, subscription_id: int, update: SubscriptionUpdate
) -> Optional[SubscriptionResponse]:
